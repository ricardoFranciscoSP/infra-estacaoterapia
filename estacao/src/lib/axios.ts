...
export const testApiConnection = async (): Promise<{ success: boolean; message: string; baseURL: string }> => {
    // Tenta o endpoint /health primeiro (endpoint oficial de health check)
    ...
    try {
        ...
        if (response.ok) {
            ...
            // Mesmo que não seja 200, se recebeu resposta, a API está acessível
            return {
                success: true,
                message: `API está acessível (status: ${response.status})`,
                baseURL
            };
        }
    } catch (error) {
        ...
        return {
            ...
            message: isNetworkError
                ? `Não foi possível conectar à API em ${baseURL}. Verifique se a API está rodando e acessível.`
                : `Erro ao conectar à API: ${errorMessage}`,
            baseURL
        };
    }
};

// Testa conexão na inicialização (apenas no cliente)
if (typeof window !== 'undefined') {
    // Aguarda um pouco antes de testar para não bloquear o carregamento inicial
    setTimeout(() => {
        testApiConnection().then(result => {
            if (result.success) {
                console.log('✅ [Axios] Teste de conexão:', result.message);
            } else {
                console.error('❌ [Axios] Teste de conexão falhou:', result.message);
                console.error('   BaseURL configurada:', result.baseURL);
            }
        }).catch(err => {
            console.error('❌ [Axios] Erro ao testar conexão:', err);
        });
    }, 2000);
}
