...
interface ModalCancelarPsicologoSalaProps {
  isOpen: boolean;
  onClose: () => void;
  onConfirm: (data: {
    motivo: string;
...
// Motivos específicos para cancelamento (problema do paciente)
const MOTIVOS_CANCELAMENTO = [
  { value: "conexao_equipamento_paciente", label: "Conexão/equipamento do paciente (após 3 tentativas)" },
  ...
  { value: "ambiente_inadequado", label: "Ambiente inadequado do paciente (privacidade/segurança)" },
  { value: "paciente_dirigindo", label: "Paciente dirigindo / Recusa estacionar" },
  { value: "paciente_nao_compareceu", label: "Paciente não compareceu (após 10 minutos de tolerância)" },
];

const ModalCancelarPsicologoSala: React.FC<ModalCancelarPsicologoSalaProps> = ({
  isOpen,
  onClose,
  onConfirm,
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      // Toast de sucesso será exibido pelo handler (handleCancelarSala) que já tem a mensagem correta do backend
      // Reset form
      setMotivo("");
      setDocumento(null);
      setObservacao("");
      ...
      // Toast de erro será exibido pelo handler (handleCancelarSala) que já trata o erro corretamente
      // Não precisa mostrar toast aqui para evitar duplicação
    ...
    <Dialog open={isOpen} onOpenChange={(open: boolean) => !open && !isLoading && handleClose()}>
      <DialogContent className="max-w-[700px] max-h-[90vh] flex flex-col p-0">
        <DialogHeader className="px-6 py-4 bg-[#232A5C] text-white rounded-t-lg">
          <DialogTitle className="text-white">Cancelar Sessão</DialogTitle>
          <DialogDescription className="text-white/80">
            Cancelamento por problema do paciente
          </DialogDescription>
        </DialogHeader>

        <div className="flex-1 px-8 py-6 overflow-y-auto">
          <Alert className="mb-6 bg-red-50 border-red-200">
            <AlertTitle className="text-gray-800">Cancelamento por problema do paciente</AlertTitle>
            <AlertDescription className="text-gray-700">
              Esta ação irá cancelar a sessão devido a problemas do paciente. O repasse financeiro será executado normalmente e a sessão NÃO será devolvida ao saldo do paciente.
            </AlertDescription>
          </Alert>
          ...
          <div className="mb-5 space-y-2">
            ...
            <Textarea
              ...
              rows={4}
              placeholder="Descreva detalhes adicionais sobre o motivo do cancelamento..."
...
export default ModalCancelarPsicologoSala;
