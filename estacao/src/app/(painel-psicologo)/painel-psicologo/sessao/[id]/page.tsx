...
export default function SessaoPage() {
  ...
  const { reservaSessao, refetch: refetchReservaSessao } = useReservaSessao(id);
  ...
  // 🎯 Gera tokens automaticamente ao entrar na sessão (mesma lógica do paciente)
  // Verifica se tem token, se não houver, gera
  useEffect(() => {
    ...
    // Se ambos os tokens já existem, não precisa gerar
    if (hasPatientToken && hasPsychologistToken) {
      hasAutoGeneratedTokensRef.current = true;
      return;
    }

    // Se algum token estiver faltando, gera ambos
    hasAutoGeneratedTokensRef.current = true;
    console.log('[SessaoPage Psychologist] Tokens ausentes, gerando automaticamente...', {
      hasPatientToken,
    ...
    checkAndGenerateTokens(id, async () => {
      ...
      hasAutoGeneratedTokensRef.current = false; // Permite tentar novamente se falhar
    });
  ...
  const markJoinedAtOnClick = useCallback(async () => {
    if (!id) return;
    const alreadyJoined = (reservaSessao as Partial<{
  ...
  const handleEntrarSessao = async () => {
    ...
    try {
      ...
      // 🎯 Mesma lógica do paciente: verifica se tem token, se não houver, gera
      ...
      try {
        await markJoinedAtOnClick();
      } catch (error) {
        console.warn('[SessaoPage Psychologist] Falha ao atualizar PsychologistJoinedAt:', error);
      }
          ...
          <div className="flex flex-col items-center order-1 lg:order-2">
            ...
            <div className="w-full max-w-sm space-y-3">
              <button
                ...
                className={`w-full h-12 rounded-lg text-white text-lg font-medium transition-all duration-200 shadow-md ${
                  !isValidatingAccess
              ...
              <button
                ...
                className="w-full h-11 rounded-lg bg-[#25D366] hover:bg-[#128C7E] text-white text-base font-medium transition-all duration-200 shadow-sm hover:shadow-md"
              >
                Suporte
              </button>
              ...
              <button
                onClick={() => router.back()}
                className="w-full h-11 rounded-lg border-2 border-[#8494E9] text-[#8494E9] bg-transparent hover:bg-[#8494E9] hover:text-white text-base font-medium transition-all duration-200"
              >
                Cancelar
              </button>
            </div>
