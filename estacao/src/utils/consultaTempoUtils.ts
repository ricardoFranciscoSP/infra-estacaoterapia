...
/**
 * Calcula o tempo decorrido dos 50 minutos a partir do ScheduledAt
 * @param scheduledAt - Data/hora do início da consulta no formato 'YYYY-MM-DD HH:mm:ss'
 * @param date - Data da consulta (fallback)
 * @param time - Horário da consulta (fallback)
 * @returns Objeto com tempo decorrido formatado e se está dentro do período de 50 minutos
 */
export function calcularTempoDecorrido50Minutos(
  scheduledAt?: string | null,
  date?: string | null,
  time?: string | null
): { tempoFormatado: string; estaDentroDoPeriodo: boolean; segundosDecorridos: number } {
  try {
    let inicioConsulta: number | null = null;
    ...
    if (scheduledAt) {
      try {
        ...
        console.error('[calcularTempoDecorrido50Minutos] Erro ao parsear ScheduledAt:', error);
      }
    }
    ...
    if (!inicioConsulta) {
      return { tempoFormatado: '', estaDentroDoPeriodo: false, segundosDecorridos: 0 };
    }
    ...
    // Formata o tempo decorrido para exibir (MM:SS)
    const minutosDecorridos = Math.floor(segundosDecorridos / 60);
    const segundosRestantesDoMinuto = segundosDecorridos % 60;
    const tempoFormatado = `${String(minutosDecorridos).padStart(2, '0')}:${String(segundosRestantesDoMinuto).padStart(2, '0')}`;
    ...
    console.error('[calcularTempoDecorrido50Minutos] Erro ao calcular tempo:', error);
    return { tempoFormatado: '', estaDentroDoPeriodo: false, segundosDecorridos: 0 };
  }
}
...
export function isConsultaDentro50MinutosComScheduledAt(
  ...
  const { estaDentroDoPeriodo } = calcularTempoDecorrido50Minutos(scheduledAt, date, time);
  return estaDentroDoPeriodo;
}

/**
 * Verifica se a consulta já começou (permitindo entrada na hora exata)
 */
export function isConsultaIniciada(
  scheduledAt?: string | null,
  date?: string | null,
  time?: string | null
): boolean {
  try {
    ...
    if (scheduledAt) {
      try {
        ...
        console.error('[isConsultaIniciada] Erro ao parsear ScheduledAt:', error);
      }
    }
    ...
    // Consulta iniciou se o tempo atual é maior ou igual ao início
    return agoraTimestamp >= inicioConsulta;
  } catch (error) {
    console.error('[isConsultaIniciada] Erro ao verificar:', error);
    return false;
  }
}
...
export function shouldEnableEntrarConsulta(params: {
  ...
  // Permite entrar na consulta quando ela começar (na hora exata ou dentro de 50 minutos)
  return isConsultaIniciada(scheduledAt, date, time) && 
         (isConsultaDentro50MinutosComScheduledAt(scheduledAt, date, time) || 
